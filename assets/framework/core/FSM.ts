...
class CustomState extends State {
    ...
    constructor(target, id, name, pattern) {
        super(id, name);
        let enterName = cc.js.formatStr(pattern, "enter", this.name)
        let updateName = cc.js.formatStr(pattern, "update", this.name)
        let exitName = cc.js.formatStr(pattern, "exit", this.name)
