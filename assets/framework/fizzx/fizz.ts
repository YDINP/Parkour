import { FizzBodyType } from "./components/FizzBody";
...
/** fixed oneway platform /// when fall from it ,engine will fix it to platform ,which is not desired   */
const vy_min = -250;
const vy_max = 30;
const max_fixPosY = 42;
...
export default class Fizz {
    ...
    static setPartionQuad(partionTree) {
        quad = partionTree
        Fizz.quad = partionTree;
    }
    ...
    static findShape(s) {
        let t = s.list
        if (s == null || s.list == null) return -1;
        return t.indexOf(s);
        //------------------------------------------------------------------------------//
    ...
    static getBodies(bodyType) {
        return bodyTypeToArray[bodyType]
    }

    static changeBodyType(body, toType) {
        Fizz.removeShape(body);
        let to = Fizz.getBodies(toType);
        to.push(body);
        body.list = to;
        this.insertPartition(body);
    }

    static addShapeType(proxy, bodyType, shapeType, ...params) {
        let func = screate[shapeType]
        if (func) {
    ...
    static solveCollision(a, b, nx, ny, pen) {
        ...
        if (!b.slip && b.shape == ShapeType.line && Math.abs(nx) != 1) {
        ...
        if (b.oneWay && sy > max_fixPosY && (vy_max > vy || vy > vy_min))
            return;
    ...
    static collision(d, b, dt) {
        if (b.enabled == false) return;
        // track the number of collision checks (optional)
        if (this.shouldCollideCallback == null || this.shouldCollideCallback(d, b)) {
            nchecks = nchecks + 1
            let nx = 0, ny = 0, pen = 0;
            if (b.isTrigger) {
                let isCol = Shapes.fasttest(d, b)
                // no need calc pen 
                if (isCol) pen = 1;
            } else {
                [nx, ny, pen] = stest(d, b, dt)
            }
            if (!pen) {
                return []
            }
            ...
            let rb = true;
            if (d.onCollide) {
                rd = d.onCollide(b, nx, ny, pen)
            }
            if (b.onCollide) {
                rb = b.onCollide(d, -nx, -ny, pen)
            }
            if (!b.isTrigger) {
                if (gravityy != 0 && ny == 1) {
                    d.fall_y = d.y;
            ...
            if (rd == true && rb == true) {
                this.solveCollision(d, b, nx, ny, pen)
            }
        }
    }
    ...
    static setMaxSpeed(max) {
        maxSpeed = max;
    }


    static retrieve(rect, bodyType = FizzBodyType.Static) {
        if (quad) {
            return quad.retrieve(rect);
        }
        return bodyTypeToArray[bodyType]
    }
