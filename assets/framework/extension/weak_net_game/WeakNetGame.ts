...
interface LoginRet {
    errno: number,
    errmsg: string,
    data: { token: string, userId: string, openId: string }
}
...
export default class WeakNetGame {
    ...
    private static requestConfig(): Promise<void> {
    ...
    //share_config_url
    private static requestShareConfig(): Promise<void> {
        ...
        if (!this.isInit) {
            ...
            if (!this.serverConfig.is_local_game) {
                ...
                if (this.serverConfig.is_normal_login) {
                    ...
                    await WeakNetGame.login(userId)
                }
                if (CC_WECHATGAME) {
